...
class ShadowEngineTest {
    ...
    fun `la previsione oraria trova l'ombra del pomeriggio a est dell'edificio`() {
        ...
        val forecast = ShadeTimeline.compute(
            ...
            to = day.plus(1, DateTimeUnit.DAY).atStartOfDayIn(zone),
            stepMinutes = 5,
        )

        val morning = LocalDateTime(day, LocalTime(8, 0)).toInstant(zone)
        val afternoon = LocalDateTime(day, LocalTime(17, 0)).toInstant(zone)
    ...
    fun `senza sole in arrivo la previsione non inventa un orario`() {
        ...
        val forecast = ShadeTimeline.compute(
            ...
            obstacles = listOf(building),
            location = rome,
            from = night.atStartOfDayIn(zone),
            to = night.plus(1, DateTimeUnit.DAY).atStartOfDayIn(zone),
            stepMinutes = 10,
        )

        assertNull(forecast.nextSunStart(LocalDateTime(night, LocalTime(9, 0)).toInstant(zone)))
    }
}
