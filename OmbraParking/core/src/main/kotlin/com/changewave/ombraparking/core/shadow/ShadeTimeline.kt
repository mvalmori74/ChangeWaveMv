...
data class ShadeForecast(
    ...
    fun qualityAt(instant: Instant): ShadeQuality? =
        slots.firstOrNull { !instant.isBefore(it.start) && instant.isBefore(it.end) }?.quality
    ...
    fun shadeEndsAfter(instant: Instant): Instant? {
        val index = slots.indexOfFirst { !instant.isBefore(it.start) && instant.isBefore(it.end) }
...
object ShadeTimeline {
    ...
    fun compute(
        ...
        if (!to.isAfter(from)) return ShadeForecast(point, emptyList(), stepMinutes)
