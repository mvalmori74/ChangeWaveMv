...
object SunTimes {
    ...
    fun forDay(date: LocalDate, zone: ZoneId, location: LatLng): DayLight {
        ...
        var current = previous.plusSeconds(SAMPLE_MINUTES * 60)
        while (current <= dayEnd) {
            ...
            current = current.plusSeconds(SAMPLE_MINUTES * 60)
        }
    ...
    private fun refineNoon(approximate: Instant, location: LatLng): Instant {
        var low = approximate.minusSeconds(SAMPLE_MINUTES * 60)
        var high = approximate.plusSeconds(SAMPLE_MINUTES * 60)
        repeat(40) {
            val span = high.toEpochMilli() - low.toEpochMilli()
            val first = Instant.ofEpochMilli(low.toEpochMilli() + span / 3)
            val second = Instant.ofEpochMilli(high.toEpochMilli() - span / 3)
        ...
        return Instant.ofEpochMilli((low.toEpochMilli() + high.toEpochMilli()) / 2)
    }
    ...
    private fun refine(before: Instant, after: Instant, location: LatLng, rising: Boolean): Instant {
        ...
        repeat(20) { // ~0.6 s di precisione partendo da un intervallo di 10 minuti
            val mid = Instant.ofEpochMilli((low.toEpochMilli() + high.toEpochMilli()) / 2)
