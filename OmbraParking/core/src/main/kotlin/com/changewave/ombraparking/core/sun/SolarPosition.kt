...
data class SunPosition(
    ...
    fun shadowLength(heightMeters: Double, maxLength: Double = MAX_SHADOW_LENGTH_M): Double {
        ...
        val length = heightMeters / tan(Math.toRadians(elevationDegrees))
...
object SolarPosition {
    ...
    fun at(instant: Instant, location: LatLng): SunPosition {
        ...
        val anomRad = Math.toRadians(geomMeanAnomSun)
        ...
        val obliquityCorr = meanObliquity + 0.00256 * cos(Math.toRadians(omega))

        val declination = Math.toDegrees(
            asin(sin(Math.toRadians(obliquityCorr)) * sin(Math.toRadians(sunAppLong)))
        )

        val varY = tan(Math.toRadians(obliquityCorr / 2)).let { it * it }
        val meanLongRad = Math.toRadians(geomMeanLongSun)
        ...
        val minutesUtc = (instant.toEpochMilli().mod(86_400_000L)) / 60_000.0
        ...
        val azimuth = if (abs(cos(Math.toRadians(zenith))) > 0.99999 || abs(cos(latRad)) < 1e-9) {
            // Sole allo zenit o osservatore ai poli: l'azimut è indeterminato.
            if (location.latitude > 0) 180.0 else 0.0
        } else {
            val cosAz = ((sin(latRad) * cos(Math.toRadians(zenith))) - sin(declRad)) /
                (cos(latRad) * sin(Math.toRadians(zenith)))
            val az = Math.toDegrees(acos(cosAz.coerceIn(-1.0, 1.0)))
